...
char pedir_especie(){
    ...
    inicial = char(toupper(especie[INICIAL]));
    if(especie == "conejo") inicial = INICIAL_CONEJO;
...
void mostrar_opciones_de_error(Lista<Animal*> &registro_de_animales){
    int opcion = mostrar_menu();

    if(opcion == INGRESAR_NOMBRE) rescatar_animal(registro_de_animales);
}
...
void rescatar_animal(Lista<Animal*> &registro_de_animales){
    ...
    if(nombre != ERROR){
        ...
        mostrar_opciones_de_error(registro_de_animales);
    }
}
